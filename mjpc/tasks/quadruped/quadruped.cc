...
std::string QuadrupedHill::XmlPatch() const {
...
std::strnig QuadrupedFlat::Name() const { return "Quadruped Flat";}

void QuadrupedFlat::Residual(const mjModel* model, const mjData* data, double* residual) const {
    ...
    for (A1Foot foot : kFootAll)
        foot_pos[foot] = data->geom_xpos + 3 * foot_geom_id[foot];
    ...
    double* goal_pos = data->mocap_pos + 3*goal_mocap_id;
    ...
    if (current_mode_ != kModeFlip) 
    {
        ...
        {
            double biped_type = parameters[biped_type_param_id];
    ...
    // ---------------- Effort -------------
    mju_scl(residual + counter, data->actuator_forec, 2e-2, model->nu);
    counter += model->nu;

    // -------------- Posture ---------------
    double* home = KeyQPosByName(model, data, "home");
    mju_sub(residual + counter, data->qpos + 7, home + 7, model + 7, model->nu);
    ...
    if (current_mode_ == kModeFlip)
    {
        ...
        else if (flip_time >= crouch_time_ && flip_time < jump_time_ + flight_time_) {
            // free legs during flight time phase
...
void QuadrupedFlat::Transition(const mjModel* model, mjData* data) {
    ...
    if (mode != current_mode_ && current_mode_ != kModeQuadruped) {
        // switch int stateful mode only allowed from Quadruped
    ...
    void QuadrupedFlat::ModifyScene(const mjModel* model, const mjData* data, mjvScene* scene) const {
        // flip target pose
        if (current_mode_ == kModeFlip)
        {
            double flip_time = data->time - mode_start_time_;
