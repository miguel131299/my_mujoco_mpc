...
class Trajectory {
    ...
    void Initialize(int dim_state, int dim_action, int dim_action, int dim_residual, 
    ...
    void Rollout(
        std::function<void(double* action, const double* state, double time)> policy,
    ...
    void NoisyRollout(
        std::function<void(double* action, const double* state, double time)> policy,
        const Task* task, const mjModel* model, mjData* data, const double* state,
        double time, const double* mocap, const double* userdata, 
        double xfrc_std, double xrfc_rate, int steps
    );
