...
class Planner {
    ...
    // return trajectory with the best total return, or nullptr if no planning
...
// additional optional interface for planners that can produce several policy proposals
class RankedPlanner : public Planner {
    public:
    virtual ~RankedPlanner() = default;

    // optimize policies, but rather than picking the best, generate up to 
    // ncandidates. Returns number of candidates created. Only called from the planning thread
    ...
    // set actions from the candidate policy. Only called from the planning thread
